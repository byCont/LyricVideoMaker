...
export function registerPreviewHandlers({ getUserDataPath, previewWorkerClient }: IpcDeps) {
  ipcMain.handle("preview:render-frame", async (_event, request: RenderPreviewRequest) => {
    return await previewWorkerClient.renderFrame(request, getUserDataPath());
  });
