...
(() => {
  ...
  function SceneRoot({
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      "div",
      {
        ...
        children: config.components.map((comp) => {
  ...
  function buildImageInitialState(options, video, timeMs, resolvedUrl) {
  ...
  function buildShapeInitialState(options, video, timeMs) {
    const transformStyle = computeTransformStyle(options, {
      width: video.width,
      height: video.height
    });
  ...
  function buildStaticTextInitialState(options, video, timeMs, metadata = {}) {
    const transformStyle = computeTransformStyle(options, {
      width: video.width,
      height: video.height
    });
    ...
    containerStyle.justifyContent = options.textAlign === "left" ? "flex-start" : options.textAlign === "right" ? "flex-end" : "center";
    containerStyle.padding = \`\${options.paddingVertical}px \${options.paddingHorizontal}px\`;
    containerStyle.boxSizing = "border-box";
    const filter = buildEffectFilter2(options);
    if (filter !== "none") {
  ...
  function buildEffectFilter2(_options) {
    return "none";
  }
  ...
  function getEqualizerLayout(options, video) {
  ...
  function getEqualizerTransformOptions(options) {
    return {
      x: options.x ?? 0,
      y: options.y ?? 0,
  ...
  function getEqualizerStaticValues(instanceId, options, barCount, video = { width: 1920, height: 1080 }) {
  ...
  function getLyricContainerStyle(video, options) {
    return computeTransformStyle(getLyricTransformOptions(options), video);
  }
  function getLyricContainerPixelWidth(video, options) {
    return Math.max(1, video.width * (options.width ?? 100) / 100);
  }
  function getLyricTransformOptions(options) {
    return {
      x: options.x ?? 0,
      y: options.y ?? 0,
      width: options.width ?? 100,
